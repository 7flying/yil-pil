...
var requireJsRuntimeConfig = vm.runInNewContext(fs.readFileSync('static/app/require.config.js') + '; require;');
    requireJsOptimizerConfig = merge(requireJsRuntimeConfig, {
        ...
        include: [
            ...
            'components/post-list/post-list',
...
gulp.task('css', function () {
    var bowerCss = gulp.src('static/bower_modules/components-bootstrap/css/bootstrap.min.css')
            .pipe(replace(/url\((')?\.\.\/fonts\//g, 'url($1fonts/')),
        appCss = gulp.src('static/css/*.css'),
        combinedCss = es.concat(bowerCss, appCss).pipe(concat('css.css')),
        fontFiles = gulp.src('./static/bower_modules/components-bootstrap/fonts/*', { base: './static/bower_modules/components-bootstrap/' });
